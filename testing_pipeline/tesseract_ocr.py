...
class Predictor(object):
    ...
    def inference(self, img):
        ...
        with torch.no_grad():
            ...
            outputs = [out[out[:, 4] >= 0.1] for out in outputs]
            print("Infer time: {:.4f}s".format(time.time() - t0))
        return outputs, img_info

def predict_and_visualize(img_path, exp_file, ckpt_file, class_names, device='cpu', conf_thres=0.1):
...
def recognize_text_in_bboxes(image, bboxes, ratio):
    results = []
    for bbox in bboxes:
        ...
        results.append((bbox, text.strip()))
    print(f'Results eps >> {results}')
    return results
...
def draw_ocr_results(image, results, font_path):
    ...
    for bbox, text in results:
        ...
        if text:
            draw.text((x_min, y_min - 25), text, fill="green", font=font)

    return cv2.cvtColor(np.array(pil_image), cv2.COLOR_RGB2BGR)


img_path = "../datasets/en_license_plate_for_ocr/Cars1.png"  # replace with your image pathYOLOX/datasets/preprocessed_ocr_dataset/images/
...
# Initialize OCR
# No need for PaddleOCR initialization
...
if outputs[0] is None:
    ...
    # Print OCR results
    print(f"OCR results: {ocr_results[0][1]}")
